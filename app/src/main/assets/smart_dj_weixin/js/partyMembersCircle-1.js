    ...
    if(thumType==0){//调用取消点赞接口
        ...
        $.get(url,{id:thumNocan}, function(data) {
            if (data.resultCode && data.resultCode == 200) {
                ...
                if(len==1){
                    ...
                    for(var i=0;i<len;i++){
                        var thumNo_s =a.eq(i).attr('data-cirId');
        ...
        $.get(url,{resId:resId,userId:getCookie('userId'),partyMemberId:getCookie('partyMemberId')}, function(data) {
            if (data.resultCode && data.resultCode == 200) {
                ...
                $(obj).html('取消');
                var html='<a href="#" class="reply-who" data-thumNo='+thumNo+'>陈磊</a>';
...
//展示全文
function show(obj){
    $(obj).siblings('.hideAll').css('display','block');
    $(obj).parents().prev('.hiddenAll').css('display','block');
    $(obj).css('display','none');
    $(obj).siblings('.hideAll').on('tap',function(){
        $(obj).prev('.lookAll').css('display','block');
...
function circleList(){
    ...
    $.get(url,{cateCode: 5007,pageSize:100}, function(data) {
        ...
        if(len==0){
            ...
            for(var i=0;i<len;i++){
                ...
                if(parseInt(partyMemberId)== partyBranch){
...
function comSend(obj){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            html +='<a class="reply-who" href="javascript:;">陈磊</a>：';
            html +=comment;
            html +='</div>';
            var lenList = $('#circleCon li').length;
            for(var i=0;i<lenList;i++){
